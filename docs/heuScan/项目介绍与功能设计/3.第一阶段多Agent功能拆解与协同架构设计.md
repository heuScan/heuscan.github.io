...
| Agent | 在聚合后端中的定位 | 主要输入 | 主要输出 |
| --- | --- | --- | --- |
| Enterprise Intel Agent | 企业情报模块 | 公司名、备案线索、外部情报响应 | 企业实体、关系边 |
| Asset Discovery Agent | 基础发现模块 | 域名、IP、路径词典 | 资产发现事件 |
| Fingerprint Agent | 指纹识别模块 | HTTP 响应、页面片段、服务特征 | 技术栈标签 |
| JS Semantic Agent | JS 解析模块 | JS 文件、HTML、AST 结果 | API / 参数 / 外联线索 |
| Dynamic Dictionary Agent | 扩展词典模块 | 企业别名、指纹、JS 路径 | 扫描词典 |
| Graph & Topology Agent | 图谱聚合模块 | 实体、资产、线索、证据 | 图谱节点、拓扑边 |
| Value Assessment Agent | 价值评估模块 | 图谱关系、资产特征、风险线索 | 优先级标签 |
| Report Agent | 报告输出模块 | 图谱快照、评分结果、证据索引 | 报告与摘要 |
| Review Agent | 复核模块 | 高风险发现、争议合并、报告草稿 | 复核结论 |
| Passive Traffic Audit Agent | 流量审查模块 | HTTP 流量样本 | 风险线索 |
| File Intel Agent | 文件情报模块 | 可下载文件与提取文本 | 文件线索 |
| Chain Hint Agent | 链路提示模块 | 图谱事实、高价值节点 | 假设链路 |
